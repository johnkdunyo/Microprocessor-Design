...
public class Assembler
{
	...
	public static String getCodeForAlu(String inst1, String inst2, String opcode, String[] args, int num)
	{
		String hex = "";
		if( args.length == 2 && isRegister(args[0]) )
		{
			...
			{
				int regsTmp = Integer.parseInt(getRegisterCode(args[0]) + getRegisterCode(args[1]), 2);
			...
			else
			{
				...
				{
					int regsTmp = Integer.parseInt(getRegisterCode(args[0]) + "00", 2);
					String regs = (regsTmp > 9) ? ""+((char)(regsTmp+87)) : ""+regsTmp;
	...
	public static String generateHexCode(String opcode, String[] args,int num,Map<String, Integer> tabLabels)
	{
		String hex = "";
		...
		{
			case "or":
			hex = getCodeForAlu("0","0","OR",args,num);
			break;
			case "not":
			...
			break;
			case "and":
			hex = getCodeForAlu("2","2","AND",args,num);
			break;
			case "xor":
			hex = getCodeForAlu("3","3","XOR",args,num);
			break;
			case "add":
			hex = getCodeForAlu("4","4","ADD",args,num);
			break;
			case "sub":
			hex = getCodeForAlu("5","5","SUB",args,num);
			break;
			case "mult":
			hex = getCodeForAlu("6","6","MULT",args,num);
			break;
			case "div":
			hex = getCodeForAlu("7","7","DIV",args,num);
			break;
			case "mov":
			hex = getCodeForAlu("8","9","MOV",args,num);
			break;
			case "load":
			...
			if(args.length == 1)
			{
				...
				{
					int regsTmp = Integer.parseInt("00" + getRegisterCode(args[0]), 2);
					String regs = (regsTmp > 9) ? ""+((char)(regsTmp+87)) : ""+regsTmp;
					hex = "c" + regs + "ff";
				}
				else
			...
			while(asm.getFilePointer() < asm.length())
			{
				...
				{	
					...
					if(!isLabel)
					{
						...
						String instruction = generateHexCode(tokens[0],ops,lineCount,tabLabels);
	...
	public static void main(String[] args)
	{
		...
		System.out.println("-----------------------------------------------");
		System.out.println("16 Bits microprocessor assembler by Computer Engineering Group 1");
		System.out.println("-----------------------------------------------");
